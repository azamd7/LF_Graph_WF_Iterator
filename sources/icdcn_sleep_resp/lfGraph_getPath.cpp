...
bool ContainsV(int key){
 ...
 bool ContainsVPlus(vlist_t ** n1, vlist_t ** n2, int key1, int key2){
	     ...
	     locateVPlus(Head, &pred2, &curr2, key2); // looking for key2 only if key1 present
	     ...
	     locateVPlus(Head, &pred1, &curr1, key1); // looking for key1 only if key2 present
             if((!curr1->vnext.load()) || curr1->val != key1)
	        return false; // key1 is not present in the vertex-list
...
Snap_VNode* snapshot(){
    ...
    snap_elist * last_snape = nullptr;


    
    vlist_t * vnode = vhead->vnext;
    while(vnode->val != INT_MAX ){
        if(!is_marked_ref((long)vnode->vnext.load())){   
            last_snapv = add_snap_vertex(vnode , last_snapv );
            last_snape = last_snapv->enext;
            elist_t * enode = vnode->enext.load()->enext;
            
            while(enode->val != INT_MAX){
                //if edge is not marked and dest vertex is not marked
                if(!is_marked_ref((long)enode->enext.load()) and !is_marked_ref((long)enode->pointv.load()->vnext.load())){
                    last_snape = add_snap_edge(enode , last_snape);
                }
                enode = (elist_t *) get_unmarked_ref((long)enode->enext.load());
            }
        }
        vnode = (vlist_t *) get_unmarked_ref((long)vnode->vnext.load());
    }

    //remove snap edges whose destination vertex is not present in the snapshot
    Snap_VNode * snap_vdest = nullptr;
    Snap_VNode * v_snap = sg_copy_head-> vnext;
    while(v_snap != nullptr){
        //verify all the edges of the node if the corresponding dest vertex is present in the snapshot
        Snap_ENode * e_snap = v_snap->enext->enext;
        Snap_ENode * esnap_prev = v_snap->enext;
        snap_vdest = sg_copy_head->vnext;
...
bool compare_snapshot(snap_vlist *snap1_head, snap_vlist *snap2_head){
    ...
    snap_vlist * vsnap2 = snap2_head->vnext;

    while(vsnap1 != nullptr && vsnap2 != nullptr){
        if(vsnap1->pointv != vsnap2->pointv)
            return false;
        
        snap_elist *enode1 = vsnap1->enext->enext;
        snap_elist *enode2 = vsnap2->enext->enext;
        
        while(enode1 != nullptr && enode2 != nullptr){
            if(enode1->pointe != enode2->pointe)
                return false;
        ...
        if(enode1 != nullptr || enode2 != nullptr){
            return false;
        }
    ...
    if(vsnap1 != nullptr || vsnap2 != nullptr){
            return false;
    }
    return true;
 ...
 void initGraph(int n, int m, int NT){
     int i,j;
     for( i=1;i<=n;i++){
         AddV(i, NT);
       }
