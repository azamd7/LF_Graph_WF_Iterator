...
void *thread_funct(void * t_args){
    ...
    long * avg_nodes = ((struct thread_args *)t_args)->avg_nodes;

    vector<double> tts;//list of time taken for snapshot
    vector<long> node_cnts;
    ...
    while(cont_exec){
        ...
        case 6:
            //snapshot
            {
                ...
                node_cnts.push_back(sc->get_node_cnt());
    ...
    if(node_cnts.size() > 0){
        long total_node_cnts = 0;
        for(long node_cnt : node_cnts){
            total_node_cnts += node_cnt;
        }
        avg_nodes[thread_num] = total_node_cnts / node_cnts.size();
...
int main(int argc, char** argv) {
    ...
    long * avg_node_cnts = new long[num_of_threads];
    ...
    for( int i=0;i < num_of_threads ;i++){
        ...
        t_args[i].avg_nodes = avg_node_cnts;
