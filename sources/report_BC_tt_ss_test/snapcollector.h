...
class SnapCollector_copy{
    ...
    float get_diameter(int tid , fstream * logfile,bool debug){
         ...
         * 
         */
        void BC_from_source(Snap_Vnode_copy *s , int v , int &path_cnt , int &path_cnt_with_v ,fstream * logfile ,bool debug){
            int source_id = s->key;
            s->path_cnt = 1;
            s->dist_from_source = 0;
        ...
        float get_BC(int v , int tid, fstream * logfile ,bool debug){
            ...
            while(vsnode != nullptr){
                if(vsnode->vnode->val != v)
                    this->BC_from_source(vsnode , v, path_cnt, v_path_cnt , logfile, debug);
 ...
 * @return  ** SnapCollector 
 */
SnapCollector * takeSnapshot(Vnode * graph_head ,  int max_threads,fstream * logfile ,bool debug, int thread_num){
