...
char* GetBFS(int key, int tid){
 ...
 BFSHead->back = NULL;
  ...
  bool status = Scan(tid, v, *BFSHead, *BFSTail); 
  if(status == true){
...
class SnapVNode{
    public:
    int key;
	SnapVNode * vnext; // pointer to the next VNode
...
class SnapGraph{
 ...
 void freeHNode(){
  ...
  void collect_ss(HSNode *h){
    ...
    HNode *t = h->next.load();
    FSet * buckets = t->buckets;
    SnapFSet * snap_buckets = Head->next->buckets;
    SnapFSet snap_b;
    FSet b;
    for(int i =0 ; i< bucket_size ; i++){
        snap_b = snap_buckets[i];
        SnapVNode * snap_vhead =  snap_b.head;
        SnapVNode * snap_vtail = snap_vhead->vnext;
        // go through each bucket and fetch the vertex bst
        // add the vertex list to snap buckets
        b = buckets[i];
        ...
        while(vnode && vnode->key != INT_MAX){
            ...
            prev_snapv = new_snap_v;

            //curr snap vertex edge details
            SnapENode * snap_ehead = new_snap_v->enext;
            SnapENode * snap_etail = snap_ehead->enext;
            SnapENode * prev_snape = snap_ehead;

            //fetch the edges for this vnode
            elist_t * ehead =  vnode->enext.load();
            for(elist_t * itNode = ehead->enext.load(); itNode->enext.load() != NULL; itNode = (elist_t*)get_unmarked_ref((long)itNode ->enext.load())){
                if(!is_marked_ref((long) itNode->enext.load()) && !is_marked_ref((long)itNode->pointv.load()->vnext.load())){  
                    //create snap Edge Node
                    SnapENode * new_snap_e = new SnapENode(itNode->key ,snap_etail, itNode);
                    prev_snape->enext =new_snap_e;
                    prev_snape = new_snap_e;
                }
            }
            vnode = (vlist_t*)get_unmarked_ref((long) vnode->vnext.load());
  ...
  /**
     ...
     * 
     */
    void BC_from_source(SnapVNode *s , int v , int &path_cnt , int &path_cnt_with_v ,int tid1,fstream * logfile){
        int source_id = s->key;
        s->path_cnt  = 1;
...
bool compare_ss_collect(SnapGraph * g1 , SnapGraph *g2){
    SnapHNode * Head1 = g1->Head->next;
    SnapHNode * Head2 = g2->Head->next;
    
    if(Head1->size != Head2->size){
        return false;
    }

    SnapFSet * buckets1 = Head1->buckets;
    SnapFSet * buckets2 = Head2->buckets;
    ...
    for( int i = 0; i< Head1->size ; i++){
        ...
        if(vnode1->key != INT_MAX )
            return false;
        if(vnode2->key != INT_MAX )
            return false;
