...
void *thread_funct(void * t_args){
    ...
    int * tps = ((struct thread_args *)t_args)->tp_list;
...
int main(int argc, char** argv) {
    ...
    int initial_vertices = (int)pow(10 , 2);
    int initial_edges = 2 * (int)pow(10, 2);
    ...
    int * tps = new int[num_of_threads];
    ...
    *continue_exec = false;
    ...
    avg_cnt = tot_cnt / num_of_threads;

    //opfile << "Avg Time : " << avg_cnt <<fixed << endl;
    //opfile << "Max Time : " << max_cnt <<fixed << endl;
